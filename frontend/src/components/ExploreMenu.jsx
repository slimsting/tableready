...
const ExploreMenu = ({ category, setCategory }) => {
  return (
    <div className=" flex flex-col gap-[20px]">
      ...
      <div className=" flex items-center justify-center gap-[30px] text-center my-[20px] overflow-x-scroll hide-scrollbar ">
          ...
          return (
            <div onClick={()=>setCategory(prev=>prev === item.menu_name ? "All" : item.menu_name)} key={index}>
              <img
                ...
                alt={item.menu_name}
              />
              <p className={`${ category === item.menu_name ?" border-4 border-x-0 border-t-0 border-b-red-500": ""} mt-[10px] color-purple-900 cursor-pointer text-[1.4vw]`}>
                {item.menu_name}
              </p>
            </div>
          );
